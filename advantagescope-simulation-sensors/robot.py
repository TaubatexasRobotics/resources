...
class MyRobot(wpilib.TimedRobot):
    def robotInit(self):
        ...
        self.motor_esquerda_frente = rev.SparkMax(55, rev.SparkLowLevel.MotorType.kBrushed)  # Motor frente esquerda
        self.motor_esquerda_tras = rev.SparkMax(51, rev.SparkLowLevel.MotorType.kBrushed)    # Motor traseira esquerda
        self.motor_direita_frente = rev.SparkMax(54, rev.SparkLowLevel.MotorType.kBrushed)   # Motor frente direita
        self.motor_direita_tras = rev.SparkMax(53, rev.SparkLowLevel.MotorType.kBrushed)     # Motor traseira direita

        # Agrupa motores para controle conjunto
        self.esquerda = wpilib.MotorControllerGroup(self.motor_esquerda_frente, self.motor_esquerda_tras)  # Grupo lado esquerdo
        self.direita = wpilib.MotorControllerGroup(self.motor_direita_frente, self.motor_direita_tras)     # Grupo lado direito
        self.direita.setInverted(True)  # Inverte motores à direita para movimento consistente

        # Configura o sistema de tração diferencial
        self.tracao = wpilib.drive.DifferentialDrive(self.esquerda, self.direita)
        self.tracao.setSafetyEnabled(False)  # Desativado para simulação (conforme seu ajuste)
        ...
        self.encoder_left_sim = EncoderSim(self.encoder_left)  # Simulador do encoder esquerdo
        self.encoder_right_sim = EncoderSim(self.encoder_right)  # Simulador do encoder direito
        ...
        self.sim = SimpleKinematicSim(
            initial_x=self.initial_x, initial_y=self.initial_y, initial_yaw_deg=self.initial_yaw_deg,  # Posição inicial (m) e orientação (graus)
        ...
        self.climber_root = self.mech.getRoot("ClimberRoot", 1.90, 0.1)  # Raiz do mecanismo
    ...
    def robotPeriodic(self):
        ...
        SmartDashboard.putNumber("Gyro Yaw", self.gyro.getAngle())  # Orientação do giroscópio (graus)
        SmartDashboard.putNumber("Encoder Esquerdo (pulsos)", self.encoder_left.get())  # Pulsos do encoder esquerdo
        SmartDashboard.putNumber("Encoder Direito (pulsos)", self.encoder_right.get())  # Pulsos do encoder direito
        SmartDashboard.putNumber("Sensor de Distância (m)", self.sim.distance_sensor)  # Distância simulada (m)
        SmartDashboard.putNumber("Distância Percorrida (m)", (self.encoder_left.getDistance() + self.encoder_right.getDistance()) / 2)  # Média dos encoders
        SmartDashboard.putNumber("Posição X (m)", self.sim.pose.X())  # Posição X no campo
        SmartDashboard.putNumber("Posição Y (m)", self.sim.pose.Y())  # Posição Y no campo
        SmartDashboard.putNumber("Orientação (graus)", -self.sim.pose.rotation().degrees())  # Orientação do robô

        # Atualiza o estado do mecanismo (climber e intake) no AdvantageScope
        self.climber_ligament.setLength(self.climber_length)  # Define comprimento do climber
        self.intake_ligament.setAngle(self.intake_angle - 90.0)  # Ajusta ângulo da garra
        SmartDashboard.putNumber("Intake Angle", self.intake_angle)
        SmartDashboard.putNumber("Climber Length", self.climber_length)

        # Atualiza a cor da LED strip no mecanismo
        if self.led_color == "red":
            self.led_square.setColor(wpilib.Color8Bit(wpilib.Color.kRed))  # Define cor vermelha
        else:  # green
            self.led_square.setColor(wpilib.Color8Bit(wpilib.Color.kGreen))  # Define cor verde
        SmartDashboard.putString("LED Color", self.led_color)  # Exibe cor atual no SmartDashboard
        ...
        if reset_option == 1 and not self.reset_triggered:
            ...
            self.sim = SimpleKinematicSim(
                initial_x=self.initial_x, initial_y=self.initial_y, initial_yaw_deg=self.initial_yaw_deg,
                wheelbase=0.6,
                encoder_left_sim=self.encoder_left_sim,
                encoder_right_sim=self.encoder_right_sim,
                robot=self  # Passa a instância do robô
            ...
            self.reset_triggered = False  # Reseta a flag quando "Manter Posição" é selecionado
    ...
    def teleopPeriodic(self):
        ...
        velocidade = -self.joystick.getRawAxis(1)   # Eixo Y: frente (positivo) ou trás (negativo)
        rotacao = -self.joystick.getRawAxis(4)      # Eixo X do stick direito: girar esquerda/direita
        ...
        if self.joystick.getPOV() == 180 and not self.led_toggle_triggered: # DPad Down
            self.led_color = "green" if self.led_color == "red" else "red"  # Alterna entre vermelho e verde
        ...
        velocidade, rotacao, pose, orientacao, left_dist, right_dist, distance = self.sim.update(velocidade, rotacao)
    ...
    def autonomousInit(self):
        ...
        """
        self.autonomous_mode = self.selector.getSelected() # Obtém o modo autônomo selecionado
    ...
    def autonomousPeriodic(self):
        ...
        # MODO AUTÔNOMO 2 (utilizando sensores e máquina de estados)
            ...
            if self.estado == 1:
                distancia = (self.encoder_left.getDistance() + self.encoder_right.getDistance()) / 2
                if distancia < 1:
                    velocidade = 1.0
                else:
                    velocidade = 0.0
...
if __name__ == "__main__":
    import ntcore
    inst = ntcore.NetworkTableInstance.getDefault()  # Obtém a instância do NetworkTables
