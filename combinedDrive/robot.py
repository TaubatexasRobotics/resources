...
class Drivetrain(wpilib.TimedRobot):
    ...
    def teleopPeriodic(self):
        ...
        if self.mode == 0:
            self.drivetrain.arcadeDrive(self.stick.getRawAxis(xbox['left-y-stick']), self.stick.getRawAxis(xbox['right-x-stick']), True)
#Cheezy Drive
        elif self.mode == 1:
...
#Best Drive
            self.drivetrain.arcadeDrive(
                self.combined_triggered_value, self.stick.getRawAxis(xbox['left-x-stick']), True)
