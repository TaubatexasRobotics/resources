    ...
    def teleopPeriodic(self):
        ...
        if hasattr(self, "navx") and self.navx.isConnected():
            ...
            SmartDashboard.putNumber(
                "1. NavX Pitch", self.navx.getPitch()
            )  # Inclinação
            SmartDashboard.putNumber("2. NavX Roll", self.navx.getRoll())  # Rolagem
            SmartDashboard.putNumber(
                "3. NavX Yaw", self.navx.getYaw()
            )  # Guinada (-180 a 180)
            SmartDashboard.putNumber(
                "4. NavX Angle", self.navx.getAngle()
            )  # Ângulo acumulado (contínuo)
        else:
            # Caso a NavX não esteja conectada, exibe mensagem de erro
            SmartDashboard.putString(
                "5. NavX Status", "Desconectado ou não inicializado"
            )
        ...
        SmartDashboard.putNumber(
            "Right Encoder Rate (m/s)", self.right_encoder.getRate()
        )
